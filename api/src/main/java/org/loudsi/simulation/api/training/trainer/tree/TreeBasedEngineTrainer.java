...
public class TreeBasedEngineTrainer<Context, Config> extends AbstractControlledRunnable implements IEngineTrainer<Config, Node<LearningRunsResult<Config>>> {
    ...
    private final ITrainingRunner<Context, Config> trainingRunner;
    ...
    public TreeBasedEngineTrainer(IGeneticManager<Context, Config> manager,
                                  IGeneticConfigurationGenerator<Config> configurationGenerator,
                                  ITrainingRunner<Context, Config> trainingRunner,
    ...
    protected void doRunnableLogic(int cycle) throws InterruptedException {
        ...
        List<Node<LearningRunsResult<Config>>> nodes = NodeHelper.getAllNodes(tree);

        List<Node<LearningRunsResult<Config>>> viableNodes = selectViableNodes(nodes, bestRun);

        System.out.println(nodes.size() + " nodes founds, " + viableNodes.size() + " reliable");

        //TODO check with real time spend
        //viableNodes.parallelStream().forEach(viableNode -> addChild(viableNode, manager, context));
        viableNodes.stream().forEach(viableNode -> addChild(viableNode, manager, context));

        bestRun = NodeHelper.findMax(tree, Comparator.comparing(LearningRunsResult::getScore));
        System.out.println("Best Result is : " + bestRun.getScore());
